...
#get specialty labels for each node
specialty_list = []
for node in G.nodes:
    specialty = df['specialty'][df['id'] == node]
    specialty_list.append(specialty)
specialty_df = pd.DataFrame(specialty_list)
specialty_df.to_csv('specialties.csv')
